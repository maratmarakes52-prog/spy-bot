...
# ===== КОНФИГ =====
TOKEN = "8691007369:AAFIHw-S6N-S04vZGum1gnRljJm7g24Aeag"
ADMIN_ID = 7481288398
...
def delete_from_db(msg_id, chat_id):
...
def handle_edit(message):
    ...
    if old:
        ...
        if old_text and old_text != new_text:
            ...
            bot.send_message(
                ADMIN_ID,
                f"✏️ {user_first_name} (@{user_username}) изменил(а) сообщение:\n\n"
                f"Было: {old_text}\n"
                f"Стало: {new_text}"
            )

        delete_from_db(msg_id, chat_id)
        save_message(message)
                ...
                try:
                    ...
                    if "message to delete not found" in str(e).lower():
                        ...
                        if text:
                            bot.send_message(
                                ADMIN_ID,
                                f"🗑️ {user_first_name} (@{user_username}) удалил(а):\n{text}"
                            )
                        elif file_id:
                            bot.send_message(
                                ADMIN_ID,
                                f"🗑️ {user_first_name} (@{user_username}) удалил(а) медиа"
                            )
                        delete_from_db(msg_id, chat_id)
            time.sleep(3)
        except Exception as e:
